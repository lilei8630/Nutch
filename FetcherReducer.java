      ...
      if (fiq == null) {
        ...
        if (useHostSettings) {
          ...
          try {
            ...
            if (host != null) {
              fiq = new FetchItemQueue(conf, host.getInt("q_mt", maxThreads), host.getLong("q_cd", crawlDelay), host.getLong("q_mcd", minCrawlDelay));
            }
        ...
        while (true) {
          ...
          try {
            ...
            final BaseRobotRules rules = protocol.getRobotRules(fit.url, fit.page);
              // 检测robots协议，如果该URL不被允许抓取，则不对该URL进行抓取，并进行状态标示。
    ...
    public void run() {
      ...
      try {
        while (hasMore) {
            ...
            if (System.currentTimeMillis() >= timelimit && timelimit != -1) {
            // enough .. lets' simply read all the entries from the input without processing them
