...
public class URLPartitioner implements Configurable {
  ...
  public int getPartition(String urlString, int numReduceTasks) {
      ...
      /**
       * 相当于hashCode = hashCode^seed,即按位取异或运算:相同位取0不同位取1
       * */
      hashCode ^= seed;
      /**
       * &为与运算
       * 把SelectorEntry分入不同的reduce任务中[0..numReduceTask-1]
       * */
  ...
  public static class SelectorEntryPartitioner extends Partitioner<SelectorEntry, WebPage> implements Configurable {
      ...
      private Configuration conf;
      /**
       * Partitioner<KEY, VALUE>是一个抽象类,需要实现抽象方法getPartition.
       *
       * */
      ...
      /**
       * 继承了Configurable接口,所以要实现getConf()和setConf()
       * */
      @Override
      public Configuration getConf() {
        return conf;
