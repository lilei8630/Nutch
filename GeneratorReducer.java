...
public class GeneratorReducer extends GoraReducer<SelectorEntry, WebPage, String, WebPage> {
  ...
  protected void reduce(SelectorEntry key, Iterable<WebPage> values, Context context) throws IOException, InterruptedException {
      for (WebPage page : values) {
          ...
          System.out.println("url in GeneratorReducer: "+key.url);
          //maxCount代表什么?在一个抓取列表中属于同一个Domain/Host/Ip下url的最大值
          if (maxCount > 0) {
              String hostordomain;
              if (byDomain) {
  ...
  protected void setup(Context context) throws IOException, InterruptedException {
      ...
      System.out.println("context.getNumReduceTasks(): "+context.getNumReduceTasks());
